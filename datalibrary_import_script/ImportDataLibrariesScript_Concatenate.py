...
def create_pair_path(root_dir):
    ...
    for root, dirs, files in os.walk(root_dir):
        ...
        #print files
        if len(files) > 0:
            file_list = [os.path.join(root, filename) for filename in files if 'fastq.gz' in filename and 'L007' in filename]
            file_list2 = [os.path.join(root, filename) for filename in files if 'fastq.gz' in filename and 'L008' in filename]
            if len(file_list) > 0:
    ...
    pair_filepath = [[f1[0], [f2[0] for f2 in lane_02 if str(os.path.basename(f1[0])).replace("L007","L008") in f2[0]][0]] for f1 in lane_01]
    print pair_filepath
