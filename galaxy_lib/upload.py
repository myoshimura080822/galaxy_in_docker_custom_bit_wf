...
def add_file( dataset, registry, json_file, output_path ):
    ...
    # add zip, *.gz tests
    if check_zip( dataset.path ):
        data_type = "zip"
        ext = "zip"

    # catch tar.gz or fastq.gz files
    is_gzipped, is_gzvalid = check_gzip( dataset.path )
    if is_gzipped and is_gzvalid:
        if re.search("fastq", os.path.basename(dataset.name)):
            data_type = "fastq.gz"
            ext = "fastq.gz"    
