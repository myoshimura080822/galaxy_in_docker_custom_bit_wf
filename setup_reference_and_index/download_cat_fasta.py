...
if not mountdir.endswith('/'):
    mountdir = mountdir + '/'
out_dname = mountdir + 'download_fasta'
...
def cat_fasta():
    newname =  out_dname + "/marged_fish_pepseq.fa"
    print newname
    if not os.path.exists(newname):
        with open(newname, 'w') as f:
            subprocess.Popen(["cat", out_dname + "/*.fa"], stdout=f)
            print "created fish pepseq fasta: " + newname.split('/')[-1]
    else:
        print "fasta file is already exits: " + newname.split('/')[-1]
