...
";
...
} elsif($OPTIONS{a} eq "robust") {
    
    print Rcmd "
        group_fact <- factor(names(groups))
        design <- model.matrix(~ group_fact, data = DGE\$samples)
        colnames(design) <- sub(\"group_fact\", \"\", colnames(design))
        dw <- estimateGLMRobustDisp(DGE,design=design, prior.df=10, maxit=6)
        pdf(file=\"$OPTIONS{e}/Tagwise_Dispersion_vs_Abundance.pdf\")
        plotBCV(dw, cex=0.4)
        dev.off()
    ";
    ...
    print Rcmd "
        ...
        lrw <- glmLRT(fw,coef=seq(2, ncol(design)))
    ";
    
    if(defined $OPTIONS{n}) {
        print Rcmd "
            tableab <- data.frame(ID=rownames(fw\$fitted.values), fw\$fitted.values, stringsAsFactors=F)
            write.table(tableab, \"$OPTIONS{n}\", quote=F, sep=\"\\t\", row.names=F)
        ";
    }
...
if($OPTIONS{a} eq "robust") {

    print Rcmd "
        options(digits = 6)
        tab <- NULL
        tab_tmp <- topTags(lrw, n=Inf, adjust.method=\"$OPTIONS{f}\")[[1]]
